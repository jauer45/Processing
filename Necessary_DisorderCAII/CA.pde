class CA {
  ...
  float pixel_colour_noisevert(float x, float y, float t, float o)
  {
    float result;  
    if ( o == 1 )
    {
      println("sin: ", o);
      result = map( sin( TWO_PI * ( t+scalar_field_offset_noisevert( x, y) ) ), -1, 1, 0, 1 );
    }
    else if( o == 2 )
     {
       println("tan: ", o);
       result = map( tan( TWO_PI * ( t+scalar_field_offset_noisevert( x, y) ) ), -1, 1, 0, 1 );
    }
    else
     {
       result = map( cos( PI * ( t+scalar_field_offset_noisevert( x, y) ) ), 0, 2, 2, -2 );
    }
    return (255 * result);
  }
  ...
  void render()
  {
      ...
      for(int j = margin; j < width - margin; j++)
      {
        ...
        for (int x = 0; x < cells.length; x++) 
        {
          if (cells[x] == 1) 
          {
            ...
            stroke( pixel_colour_noisevert(x, j, t, o) );
    ...
    println(frameCount, "/" , numFrames, '(', t ,')');
